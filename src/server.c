...
void *handle_client(void *arg) {
    ...
    send(client->socket, "Login: ", 7, 0);
    recv(client->socket, username, sizeof(username) - 1, 0);
    clean_input(username);
    send(client->socket, "Password: ", 10, 0);
    recv(client->socket, password, sizeof(password) - 1, 0);
    ...
    while (1) {
        ...
        if (strncmp(buffer, "join ", 5) == 0) {
            ...
            send_message_to_channel(channel_name, "A new user has joined the channel.", client->socket);
        } else if (strcmp(buffer, "leave") == 0) {
            if (strlen(client->current_channel) > 0) {
                char response[BUFFER_SIZE];
                snprintf(response, sizeof(response), "Vous avez quitté le salon %s\n", client->current_channel);
                send(client->socket, response, strlen(response), 0);
                send_message_to_channel(client->current_channel, "A user has left the channel.", client->socket);
